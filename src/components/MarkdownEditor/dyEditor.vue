...
const onInit = async () => {
    dynamicLoadLink("https://uicdn.toast.com/editor/latest/toastui-editor.min.css");

    dynamicLoadScript((<any>window).window.toastui, "https://uicdn.toast.com/editor/latest/toastui-editor-all.min.js", (err: any) => {
        if (err) {
            ElMessage.error(err.message);
            return;
        }
        props.language === "zh-CN" ? loadRemoteScript('https://uicdn.toast.com/editor/latest/i18n/zh-cn.js', () => {
            // 这里是加载完成后的回调函数
            initEditor()
        }) : initEditor()
    })
