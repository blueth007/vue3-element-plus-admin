<template>
  <div class="upload-container">
    ...
    <el-dialog v-model="dialogVisible">
      <aside>
        自动上传upload函数和mockjs有冲突，且
        <a href="https://github.com/PanJiaChen/vue-element-admin/issues/445"
          >on-progress 无法正常监听作者已知 bug</a
        >，没有进度条动画。重新改写了 http-request 函数
      </aside>
      <el-upload
        :multiple="true"
        ref="uploadRef"
...
import { ElButton, ElUpload, ElDialog } from "element-plus";
...
function checkAllSuccess() {
  return Object.keys(data.listObj).every(
    (item) => data.listObj[item].hasSuccess
  );
}
