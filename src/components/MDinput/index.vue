<template>
    <div :class="computedClasses" class="material-input__component" ref="mdInput">
        <div :class="{ iconClass: icon }">
            ...
            <input v-if="type === 'number'" v-model="currentValue" :name="name" :placeholder="fillPlaceHolder" :step="step"
                ...
                :minlength="minlength" :maxlength="maxlength" :required="required" type="number" class="material-input"
...
const { fillPlaceHolder } = toRefs(data)
const computedClasses = computed(() => {
    return {
        'material--active': data.focus,
        'material--disabled': props.disabled,
        'material--raised': Boolean(data.focus || data.currentValue) // has value
    }
})
...
function handleModelInput(event: any) {
    const value = event.target.value
    emit('input', value)
    // console.log('handleModelInput: ', mdInput.value?.parentElement.parentElement, mdInput.value?.parentElement.parentElement.parentElement)
    if (mdInput.value?.parentElement.parentElement.classList.contains('el-form-item')) {
...
function handleMdFocus(event: any) {
    data.focus = true
    emit('focus', event)
    if (props.placeholder && props.placeholder !== '') {
        data.fillPlaceHolder = props.placeholder
    }
}

function handleMdBlur(event: any) {
    data.focus = false
    emit('blur', event)
    data.fillPlaceHolder = null

    if (mdInput.value?.parentElement.parentElement.classList.contains('el-form-item')) {
        if (props.validateEvent) {
            formItem?.validate('blur') // 触发校验
            // $vm.$parent.$emit('el.form.change', [value])
            // mdInput.value?.parentElement.parentElement.parentElement.change(value)
        }
    }
}
</script>
...
.material-input__component {
    ...
    * {
    ...
    .material-input {
        font-size: $font-size-base;
        padding: $spacer $spacer calc($spacer - $apixel * 10) calc($spacer / 2);
        display: block;
        width: 100%;
        border: none;
        line-height: 1;
