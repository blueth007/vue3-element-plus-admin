...
const { article, fullscreenLoading } = toRefs(data);
onMounted(() => {
  fetchData();
});

function fetchData() {
  import("./content.js").then((result) => {
    const { title } = result.default;
    document.title = title;
    data.article = result.default;
    ...
    //     data.fullscreenLoading = false
    //     nextTick(() => {
    //         //alert('没有setTimeout一直提示在加载不能正常打印')
    //         setTimeout(() => {
    //             window.print()
    //         }, 1110)
