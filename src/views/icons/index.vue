<template>
    <div class="icons-container">
        <aside>
            <a href="https://panjiachen.github.io/vue-element-admin-site/guide/advanced/icon.html" target="_blank">Add and
                use
            </a>
            <p>element-plus icons has been improved svg-icons for
                vue3。element-plus更换字体图标为svg图。直接从iconify获取并添加el-icon类来使图标样式匹配</p>
            <p> <a href="https://fontawesome.com/v5/search" target="_blank">fontawesome</a>字体图标使用的是5.15.4.根据 i的class="
                fas/far/fab xxx"
                来从类实心(fas) 常规(far) 品牌(fab)获取图标 </p>
        </aside>
        <el-tabs type="border-card">
            ...
            <el-tab-pane label="Element-Plus Icons">
                <div v-for="(icons, index ) in elementPlusIcons" :key="icons + index">
                    <h3>{{ index }}</h3>
                    <div class="grid">
                        <div v-for="item, idx in icons" :key="item + idx"
                            ...
                            <el-tooltip placement="top">
                                ...
                                <div class="icon-item">
                                    <span class="iconify el-icon" :data-icon="'ep:' + item"></span>
                                    <span>{{ item }}</span>
                                </div>
                            </el-tooltip>
                        </div>
            ...
            <el-tab-pane label="awesomeFont Icons">
                <div class="grid">
                    <div v-for="item in aweSome" :key="item"
