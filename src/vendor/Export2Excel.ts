...
function datenum(v: any, date1904: any = undefined) {
  if (date1904) v += 1462;
  var epoch = Date.parse(v);
  return (epoch - new Date(Date.UTC(1899, 11, 30)).getTime()) / (24 * 60 * 60 * 1000);
}

function sheet_from_array_of_arrays(data: any, opts: any = undefined) {
  var ws: any = {};
  var range = {
    s: {
      c: 10000000,
...
// this is  not found ,use XLSX.utils.book_new() instead.  It's not a standard format for XLSX files.
// function Workbook(): any {
